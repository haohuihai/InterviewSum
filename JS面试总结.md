...
str.search("")//查找是否匹配，匹配成功返回位置的索引，否则返回-1
str.toLowerCase()//将整个字符串转换为小写
...
CromeBlink(基于webkit,Goole与Opera 共同开发)
...
push() reverse() shit()slice() sort()splice() toString() toLocalString() unshit()
